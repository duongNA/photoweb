...
// 			$userWithEmail = $this->User->findByEmail($this->user('email'));
			
			//if we have a user, set hasAccount
			if(!empty($this->authUser)){
				$this->hasAccount = true;
			} 
// 			elseif (!empty($userWithEmail)) {
// 				$userWithEmail[$this->User->alias]['facebook_id'] = $this->uid;
...
// 			}
			...
			elseif(empty($this->authUser) && $this->createUser) {
				...
				// hack code to create user
				$this->authUser[$this->User->alias]['username'] = $this->user('name');
				$this->authUser[$this->User->alias]['role'] = 'author';
				$this->authUser[$this->User->alias]['status'] = 0;
				$this->authUser[$this->User->alias]['gender'] = $this->user('gender');
